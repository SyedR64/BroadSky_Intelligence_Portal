/* Fair Harbor — sustainable beachwear from recycled plastic (BSP investment, Mar 2022).
   Views: overview · opportunities · benchmarks · home sales (Manhattan) · filings. Shared helpers are duplicated in bpi.js by design (no cross-module imports). */
...
const CFG = {
  ...
  soWhat: () => 'Fair Harbor is a ~$20-35M (est.) sustainable beachwear brand, 4.5 years into the BSP hold. Growth has stalled since 2021: 38.5% of variants are marked down and the women\'s line was paused. The path to exit is capital-light wholesale, licensing and B2B growth plus margin repair, not more owned stores.',
  signal: (ctx, d) => {
    ...
    return { title: 'Catalog health (live Shopify snapshot)', sub: `${ctx.fmt.num(k.active_products)} active products · ${ctx.fmt.num(k.variants)} variants · median price $${ctx.fmt.num(k.median_variant_price_usd)}`,
      body: `<div class="row gap-12" style="align-items:flex-start"><div class="kpi" style="--kc:var(--red);min-width:120px"><div class="label">Marked down</div><div class="value">${ctx.fmt.num(k.markdown_share_pct, 1)}%</div><div class="sub">${ctx.fmt.num(k.variants_marked_down_vs_compare_at)} variants</div></div><div class="grow">${ctx.charts.hbar(Object.entries(k.product_type_mix).map(([l, v]) => ({ label: l, value: v, color: /swim|rash|short/i.test(l) ? CFG.hex : '#7d8796' })), { fmt: v => ctx.fmt.num(v), labelW: 120 })}</div></div><div class="small text-2 mt-8">Swim and summer items (teal) are still ~${ctx.fmt.num(((k.product_type_mix.Swim || 0) + (k.product_type_mix.Shorts || 0) + (k.product_type_mix.Rashguard || 0)) / k.active_products * 100)}% of the catalog. Women's products: ${ctx.fmt.num(k.womens_products)}. Markdown depth this high points to inventory clean-up.</div>`,
  ...
  levers: (d, f) => [
    ...
    { t: 'Margin repair & markdown discipline', why: '38.5% of live variants are marked down and women\'s was liquidated at ~70% off. Tighter buys and fewer SKUs matter more than top-line growth for EBITDA (est. $0-4M).', href: '#/fh/filings' },
    ...
    { t: 'Clienteling in affluent NYC ZIPs', why: `Manhattan $2M+ home buyers are the brand's 45-54 coastal-weekend customer. Use the home-sales view to target the SoHo store, pop-ups and nearby doors. ${f.pe} sponsors are circling consumer brands.`, href: '#/fh/market' },
  ],
  actions: [
  ...
  marketLens: s => `In Manhattan, home to Fair Harbor's SoHo store, the median arms-length home sold for ${s.medTxt} (${s.n} sales), and ${s.lux} sales cleared $2M. These affluent households, concentrated in ${s.top.slice(0, 3).join(', ')}, are the brand's core coastal-weekend customer.`,
  marketActions: s => [
    `Clienteling: run SoHo store events and direct mail aimed at the top $2M+ neighborhoods (${s.top.slice(0, 3).join(', ')}).`,
    'Pop-ups and wholesale: put seasonal pop-ups or partner doors in the highest-density luxury ZIPs; judge them on sell-through, not traffic.',
    'Next data pull: Suffolk County (Hamptons and Fire Island, where the brand started), Westchester (Larchmont HQ) and Palm Beach County (store 2) sales.',
  ],
  hoodAction: h => h.luxShare >= 15 ? 'Luxury cluster: priority for SoHo-store clienteling, events and a pop-up test.' : h.n >= 300 ? 'High-turnover neighborhood: new-mover welcome offers (summer kit) via direct mail.' : 'Monitor; low priority for spend.',
  filingsSoWhat: "No revenue has been disclosed. Filings imply ~$20-35M of 2025 revenue and ~$0-4M EBITDA (est.) on $30.7M of BSP equity, with no funded debt found. Heavy markdowns and the women's liquidation point to margin pressure ahead of an exit.",
};
...
export default {
  ...
  tour: [
    { order: 700, hash: '#/fh/overview', caption: '<b>Fair Harbor.</b> A sustainable beachwear brand, 4.5 years into the hold, with heavy markdowns. The value lever is margin, not stores.', narration: 'For Fair Harbor, the value lever is margin repair, not more stores.', duration: 5500 },
    { order: 710, hash: '#/fh/opportunities', caption: '<b>9 capital-light growth plays</b>: wholesale doors, golf and collegiate licensing, B2B custom and international DTC.', narration: 'Nine capital-light growth plays, from wholesale doors and golf to licensing and international direct-to-consumer.', duration: 6000 },
    { order: 720, hash: '#/fh/market', caption: '<b>Manhattan home sales</b> by ZIP: where the $2M+ coastal-weekend customer lives, for clienteling around the SoHo store.', narration: 'A year of Manhattan home sales by ZIP shows where the affluent weekend customer lives, around the SoHo store.', duration: 8000 },
  ],
};
